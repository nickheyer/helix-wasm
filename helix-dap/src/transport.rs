...
mod wasm {
    use crate::{registry::DebugAdapterId, Result};
    use serde::{Deserialize, Serialize};
    use serde_json::Value;
    use std::fmt::Debug;
    ...
    impl Transport {
        pub fn start(
            _server_stdout: Box<dyn Unpin + Send>,
            _server_stdin: Box<dyn Unpin + Send>,
            _server_stderr: Option<Box<dyn Unpin + Send>>,
            _id: DebugAdapterId,
        ) -> (UnboundedReceiver<Payload>, UnboundedSender<Payload>) {
            let (_client_tx, rx) = tokio::sync::mpsc::unbounded_channel();
            let (tx, _client_rx) = tokio::sync::mpsc::unbounded_channel();
            (rx, tx)
        }
    }
}
