...
// On wasm32, clang's C ABI passes structs by pointer, but Rust's old wasm ABI
// splats them as individual arguments.  Use a pointer here to match clang.
#[cfg(target_arch = "wasm32")]
...
extern "C" {
    ...
    // On wasm32, clang passes large structs by pointer (standard C ABI),
    // but Rust's old wasm ABI splats struct fields as separate arguments.
    // Use pointers for struct args to match clang.
    #[cfg(not(target_arch = "wasm32"))]
    fn ts_parser_parse(
        parser: NonNull<ParserData>,
