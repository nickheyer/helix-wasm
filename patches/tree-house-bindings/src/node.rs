...
impl<'tree> Node<'tree> {
    ...
    #[inline]
    unsafe fn map(&self, f: unsafe fn(NodeRaw) -> NodeRaw) -> Option<Node<'tree>> {
        Node::from_raw(f(self.as_raw()))
    }
...
mod wasm_node_ffi {
    ...
    // --- Primitive-returning functions (NodeRaw passed by pointer) ---

    extern "C" {
        #[link_name = "ts_node_type"]
        fn __ts_node_type(node: *const NodeRaw) -> *const c_char;
    }
    pub(super) unsafe fn ts_node_type(node: NodeRaw) -> *const c_char {
        __ts_node_type(&node)
    }

    extern "C" {
        #[link_name = "ts_node_symbol"]
        fn __ts_node_symbol(node: *const NodeRaw) -> u16;
    }
    pub(super) unsafe fn ts_node_symbol(node: NodeRaw) -> u16 {
        __ts_node_symbol(&node)
    }

    extern "C" {
        #[link_name = "ts_node_language"]
        fn __ts_node_language(node: *const NodeRaw) -> Grammar;
    }
    pub(super) unsafe fn ts_node_language(node: NodeRaw) -> Grammar {
        __ts_node_language(&node)
    }

    extern "C" {
        #[link_name = "ts_node_is_named"]
        fn __ts_node_is_named(node: *const NodeRaw) -> bool;
    }
    pub(super) unsafe fn ts_node_is_named(node: NodeRaw) -> bool {
        __ts_node_is_named(&node)
    }

    extern "C" {
        #[link_name = "ts_node_is_missing"]
        fn __ts_node_is_missing(node: *const NodeRaw) -> bool;
    }
    pub(super) unsafe fn ts_node_is_missing(node: NodeRaw) -> bool {
        __ts_node_is_missing(&node)
    }

    extern "C" {
        #[link_name = "ts_node_is_extra"]
        fn __ts_node_is_extra(node: *const NodeRaw) -> bool;
    }
    pub(super) unsafe fn ts_node_is_extra(node: NodeRaw) -> bool {
        __ts_node_is_extra(&node)
    }

    extern "C" {
        #[link_name = "ts_node_child_count"]
        fn __ts_node_child_count(node: *const NodeRaw) -> u32;
    }
    pub(super) unsafe fn ts_node_child_count(node: NodeRaw) -> u32 {
        __ts_node_child_count(&node)
    }

    extern "C" {
        #[link_name = "ts_node_named_child_count"]
        fn __ts_node_named_child_count(node: *const NodeRaw) -> u32;
    }
    pub(super) unsafe fn ts_node_named_child_count(node: NodeRaw) -> u32 {
        __ts_node_named_child_count(&node)
    }

    extern "C" {
        #[link_name = "ts_node_end_byte"]
        fn __ts_node_end_byte(node: *const NodeRaw) -> u32;
    }
    pub(super) unsafe fn ts_node_end_byte(node: NodeRaw) -> u32 {
        __ts_node_end_byte(&node)
    }

    // --- NodeRaw-returning functions (sret: result pointer is first arg) ---

    extern "C" {
        #[link_name = "ts_node_parent"]
        fn __ts_node_parent(result: *mut NodeRaw, node: *const NodeRaw);
    }
    pub(super) unsafe fn ts_node_parent(node: NodeRaw) -> NodeRaw {
        let mut result = MaybeUninit::uninit();
        __ts_node_parent(result.as_mut_ptr(), &node);
        result.assume_init()
    }

    extern "C" {
        #[link_name = "ts_node_child"]
        fn __ts_node_child(result: *mut NodeRaw, node: *const NodeRaw, child_index: u32);
    }
    pub(super) unsafe fn ts_node_child(node: NodeRaw, child_index: u32) -> NodeRaw {
        let mut result = MaybeUninit::uninit();
        __ts_node_child(result.as_mut_ptr(), &node, child_index);
        result.assume_init()
    }

    extern "C" {
        #[link_name = "ts_node_named_child"]
        fn __ts_node_named_child(result: *mut NodeRaw, node: *const NodeRaw, child_index: u32);
    }
    pub(super) unsafe fn ts_node_named_child(node: NodeRaw, child_index: u32) -> NodeRaw {
        let mut result = MaybeUninit::uninit();
        __ts_node_named_child(result.as_mut_ptr(), &node, child_index);
        result.assume_init()
    }

    extern "C" {
        #[link_name = "ts_node_next_sibling"]
        fn __ts_node_next_sibling(result: *mut NodeRaw, node: *const NodeRaw);
    }
    pub(super) unsafe fn ts_node_next_sibling(node: NodeRaw) -> NodeRaw {
        let mut result = MaybeUninit::uninit();
        __ts_node_next_sibling(result.as_mut_ptr(), &node);
        result.assume_init()
    }

    extern "C" {
        #[link_name = "ts_node_prev_sibling"]
        fn __ts_node_prev_sibling(result: *mut NodeRaw, node: *const NodeRaw);
    }
    pub(super) unsafe fn ts_node_prev_sibling(node: NodeRaw) -> NodeRaw {
        let mut result = MaybeUninit::uninit();
        __ts_node_prev_sibling(result.as_mut_ptr(), &node);
        result.assume_init()
    }

    extern "C" {
        #[link_name = "ts_node_next_named_sibling"]
        fn __ts_node_next_named_sibling(result: *mut NodeRaw, node: *const NodeRaw);
    }
    pub(super) unsafe fn ts_node_next_named_sibling(node: NodeRaw) -> NodeRaw {
        let mut result = MaybeUninit::uninit();
        __ts_node_next_named_sibling(result.as_mut_ptr(), &node);
        result.assume_init()
    }

    extern "C" {
        #[link_name = "ts_node_prev_named_sibling"]
        fn __ts_node_prev_named_sibling(result: *mut NodeRaw, node: *const NodeRaw);
    }
    pub(super) unsafe fn ts_node_prev_named_sibling(node: NodeRaw) -> NodeRaw {
        let mut result = MaybeUninit::uninit();
        __ts_node_prev_named_sibling(result.as_mut_ptr(), &node);
        result.assume_init()
    }

    extern "C" {
        #[link_name = "ts_node_descendant_for_byte_range"]
        fn __ts_node_descendant_for_byte_range(result: *mut NodeRaw, node: *const NodeRaw, start: u32, end: u32);
    }
    pub(super) unsafe fn ts_node_descendant_for_byte_range(node: NodeRaw, start: u32, end: u32) -> NodeRaw {
        let mut result = MaybeUninit::uninit();
        __ts_node_descendant_for_byte_range(result.as_mut_ptr(), &node, start, end);
        result.assume_init()
    }

    extern "C" {
        #[link_name = "ts_node_named_descendant_for_byte_range"]
        fn __ts_node_named_descendant_for_byte_range(result: *mut NodeRaw, node: *const NodeRaw, start: u32, end: u32);
    }
    pub(super) unsafe fn ts_node_named_descendant_for_byte_range(node: NodeRaw, start: u32, end: u32) -> NodeRaw {
        let mut result = MaybeUninit::uninit();
        __ts_node_named_descendant_for_byte_range(result.as_mut_ptr(), &node, start, end);
        result.assume_init()
    }
}
