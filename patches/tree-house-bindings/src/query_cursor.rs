...
mod wasm_query_cursor_ffi {
    use super::*;

    extern "C" {
        // ABI-adjusted: NodeRaw passed by pointer on wasm32
        #[link_name = "ts_query_cursor_exec"]
        fn __ts_query_cursor_exec(self_: *mut QueryCursorData, query: &QueryData, node: *const NodeRaw);

        // These functions only use pointers/primitives — no ABI mismatch
        pub(super) fn ts_query_cursor_next_capture(
            self_: *mut QueryCursorData,
            match_: &mut TSQueryMatch,
            capture_index: &mut u32,
        ) -> bool;
        pub(super) fn ts_query_cursor_next_match(
            self_: *mut QueryCursorData,
            match_: &mut TSQueryMatch,
        ) -> bool;
        pub(super) fn ts_query_cursor_remove_match(self_: *mut QueryCursorData, match_id: u32);
        pub(super) fn ts_query_cursor_delete(self_: *mut QueryCursorData);
        pub(super) fn ts_query_cursor_new() -> *mut QueryCursorData;
        pub(super) fn ts_query_cursor_did_exceed_match_limit(
            self_: *const QueryCursorData,
        ) -> bool;
        pub(super) fn ts_query_cursor_match_limit(self_: *const QueryCursorData) -> u32;
        pub(super) fn ts_query_cursor_set_match_limit(self_: *mut QueryCursorData, limit: u32);
        pub(super) fn ts_query_cursor_set_byte_range(
            self_: *mut QueryCursorData,
            start_byte: u32,
            end_byte: u32,
        ) -> bool;
    ...
    pub(super) unsafe fn ts_query_cursor_exec(
        self_: *mut QueryCursorData,
        query: &QueryData,
        node: NodeRaw,
    ) {
        __ts_query_cursor_exec(self_, query, &node)
    }
}
#[cfg(target_arch = "wasm32")]
use wasm_query_cursor_ffi::*;
