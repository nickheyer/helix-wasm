...
/// On WASM there is exactly one thread, so all Nucleo instances must share
/// a single rayon pool (rayon forbids registering a thread in multiple pools).
#[cfg(target_arch = "wasm32")]
static WASM_POOL: std::sync::OnceLock<Arc<ThreadPool>> = std::sync::OnceLock::new();
...
impl<T: Sync + Send + 'static> Worker<T> {
    ...
    pub(crate) fn new(
        ...
        #[cfg(target_arch = "wasm32")]
        let pool = WASM_POOL.get_or_init(|| {
            Arc::new(rayon::ThreadPoolBuilder::new()
                .num_threads(1)
                .use_current_thread()
                .build()
                .expect("creating single-thread pool failed"))
        }).clone();
