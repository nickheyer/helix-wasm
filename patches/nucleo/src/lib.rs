...
impl<T: Sync + Send + 'static> Nucleo<T> {
    ...
    fn tick_inner(&mut self, timeout: u64, canceled: bool, status: pattern::Status) -> Status {
        let mut inner = if canceled {
            ...
            #[cfg(not(target_arch = "wasm32"))]
            let lock_result = self.worker.try_lock_arc_for(Duration::from_millis(timeout));
            #[cfg(target_arch = "wasm32")]
            let lock_result = self.worker.try_lock_arc();
            let Some(worker) = lock_result else {
        ...
        if running {
            ...
            #[cfg(target_arch = "wasm32")]
            self.pool
                .install(move || unsafe { inner.run(status, cleared) });
        }
