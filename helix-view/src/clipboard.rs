...
// ─── Browser clipboard callback ─────────────────────────────────────────
// Set from helix-wasm during init to wire Browser variant to
// navigator.clipboard.writeText via wasm-bindgen.

static CLIPBOARD_WRITE_FN: OnceLock<Box<dyn Fn(&str) + Send + Sync>> = OnceLock::new();

pub fn register_clipboard_writer(writer: Box<dyn Fn(&str) + Send + Sync>) {
    CLIPBOARD_WRITE_FN.set(writer).ok();
}
...
impl Default for ClipboardProvider {
    fn default() -> Self {
        if CLIPBOARD_WRITE_FN.get().is_some() {
            return Self::Browser;
        }
        Self::detect_system()
    }
...
impl ClipboardProvider {
    ...
    pub fn get_contents(&self, clipboard_type: &ClipboardType) -> Result<String> {
        ...
        match self {
            ...
            Self::Browser => Err(ClipboardError::ReadingNotSupported),
    ...
    pub fn set_contents(&self, content: &str, clipboard_type: ClipboardType) -> Result<()> {
        ...
        match self {
            ...
            Self::Browser => {
                if let Some(write_fn) = CLIPBOARD_WRITE_FN.get() {
                    write_fn(content);
                    Ok(())
                } else {
