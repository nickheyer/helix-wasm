...
mod native {
    ...
    pub fn general() -> std::io::Result<()> {
        ...
        for rt_dir in rt_dirs.iter() {
            if let Ok(path) = helix_vfs::read_link(rt_dir) {
                let msg = format!(
    ...
    fn languages(selection: Option<HashSet<String>>) -> std::io::Result<()> {
        ...
        let color =
            |s: Stylized<'static>, c: ColorSpec| if is_terminal { s.foreground(c) } else { s };
...
pub fn print_health(_health_arg: Option<String>) -> std::io::Result<()> {
    Ok(())
}
